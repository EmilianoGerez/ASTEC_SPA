...
exports.viewOrders = function(req, res) {
  var start = new Date(req.params.year, (req.params.startMonth - 1));
  var end = new Date(req.params.year, req.params.startMonth);

  var endMonth = parseInt(req.params.endMonth);

  if (!isNaN(endMonth)) {
    end = new Date(req.params.year, endMonth);
  }


  Order.find({ dateCreated: { $gte: new Date(start.toISOString()), $lt: new Date(end.toISOString()) } }).populate('client').exec(function(err, orders) {
    if (err) {
      res.status(500).send(err.message);
    }
